    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) 
        ...
        try {
            ...
            if (accountNumber == null || accountNumber.trim().isEmpty() ||
                ...
                request.getRequestDispatcher("pages/deposit.jsp").forward(request, response);
                return;
            }
            ...
            try {
                ...
                if (amount.compareTo(new BigDecimal("100000")) > 0) {
                    request.setAttribute("error", "Maximum deposit amount is $100,000");
                    request.getRequestDispatcher("pages/deposit.jsp").forward(request, response);
                    return;
                }
            ...
            if (balanceUpdated) {
                ...
                if (transactionCreated) {
                    request.setAttribute("success", 
                        "Deposit successful! Amount: $" + amount + " has been added to account " + accountNumber);
                } else {
                    request.setAttribute("error", "Deposit processed but transaction record failed");
                }
            } else {
